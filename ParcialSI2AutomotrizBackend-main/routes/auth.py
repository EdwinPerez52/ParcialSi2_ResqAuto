...
# ==========================================
@router.post("/login")
def iniciar_sesion(datos: LoginRequest, conn = Depends(get_db)):
    try:
...
def registrar_usuario(datos: RegistroUsuarioRequest, conn = Depends(get_db)):
    try:
        cursor = conn.cursor(cursor_factory=RealDictCursor)
...
def recuperar_password(datos: RecuperarPasswordRequest, conn = Depends(get_db)):
    try:
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute("SELECT id FROM usuario WHERE correo = %s", (datos.correo,))
        usuario = cursor.fetchone()
...
def reset_password(datos: ResetPasswordRequest, conn = Depends(get_db)):
    try:
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute("SELECT id FROM usuario WHERE correo = %s", (datos.correo,))
        usuario = cursor.fetchone()
...
# ==========================================
@router.post("/logout")
def cerrar_sesion(usuario: dict = Depends(verificar_auth), conn = Depends(get_db)):
    try:
        cursor = conn.cursor()
        cursor.execute("""
...
# ==========================================
@router.get("/perfil")
def obtener_perfil(usuario: dict = Depends(verificar_auth), conn = Depends(get_db)):
    try:
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute("""
